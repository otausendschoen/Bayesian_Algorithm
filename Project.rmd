...
Pair plots can help us assess potential correlations between parameters and they also provide an overview of the joint posterior distributions.
```{r, echo=FALSE}
...
Running diagnostics on the fitted model:
```{r, echo=FALSE}
nc_diag <- run_diagnostics(fit_nc, model_name = "non_cent_par")
```
...
```
For further optimization, we will run both models (centered and non-centered) and let the routine decide which performs better based on these diagnostics, and then compare the selected model against the baseline. 
...
We run diagnostics for the dense non-centered model:
```{r, echo=FALSE}
...
```
(Note: This initialization strategy is based on our model specifics and may be refined further in the future.)
...
```
*Note:* For simplicity, we approximate $\m_n$ using mu_alpha and beta (in practice, though we could also extract each $\alpha$ from $fit_cont$), anything else would have taken too much computational time. (You can find the output of this code chunk in the $Project.rmd$ file.)
...
##### Parameter Trajectories Over Hybrid Sampling Iterations

We can use this dataframe to see how the posterior mean estimates of the continuous parameters ($\mu_\alpha$, $\beta$, and $\sigma$) change at each iteration of the hybrid sampler.
```{r, echo=FALSE}
...
ggplot(hybrid_long, aes(x = iteration, y = value)) +
  ...
  theme_minimal()
```
We can see how the hybrid sampler transitions through different regions of the posterior space over time, confirming that the sampler is indeed responding to updates in $Z$ and exploring the posterior distribution rather than remaining static.
...
draws_hybrid <- as_draws_df(final_fit)  
